...
export default function CohortSubjectDashboardPage() {
  ...
  useEffect(() => {
    if (!user?.token || !subjectId) return;
    ...
    (async () => {
      try {
        setLoading(true);
        const res = await getCohortDashboard(user.schoolId, "subject", subjectId, user.token);
        if (!cancelled) {
          setData(res);
          setError(null);
        }
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-sky-400">Subject cohort</p>
        <h1 className="text-2xl font-bold text-slate-100">{data.name}</h1>
        <p className="mt-1 font-mono text-sm text-slate-500">{data.cohortId}</p>
      </div>
